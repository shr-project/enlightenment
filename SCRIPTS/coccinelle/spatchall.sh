...
if [ ! -z "$TARGET" ]; then
	...
	TARGETS="eina eet embryo evas ecore efreet edje e_dbus eeze e elementary E16 E-MODULES-EXTRA BINDINGS editje edje_viewer eio elmdentica emotion empower emprint enki enlil ensure ephoto eterm ethumb eve exalt EXAMPLES expedite exquisite imlib2 imlib2_loaders MISC $(ls PROTO | grep -v emote | while read d; do echo PROTO/$a; done) rage TEST"
fi
...
for d in $TARGETS; do
	...
	#  config.h, stdarg.h, stddef.h, Evil.h
		;;
	evas)
		call_spatch $d -I /usr/include/valgrind/ -I ${TOPDIR}/eet/src/lib/ -I src/lib/ -I src/modules/engines/buffer/ $(for i in ${TOPDIR}/evas/src/lib/{include,file,cserve}; do echo -I $i; done)
	# missing headers:
	...
	#  config.h, stddef.h
		;;
	e_dbus)
		call_spatch $d -I ${TOPDIR}/eina/src/include/ $(for i in ${TOPDIR}/ecore/src/lib/ecore*; do echo -I $i; done) $(for i in ${TOPDIR}/e_dbus/src/lib/{bluez,connman0_7x,dbus,hal,notification,ofono,ukit}; do echo -I $i; done) \
	# missing headers:
	#  config.h, stddef.h
		;;
	eeze)
	...
	#  config.h
		;;
	e)
		call_spatch $d -I ${TOPDIR}/eina/src/include/ $(for i in ${TOPDIR}/ecore/src/lib/ecore*; do echo -I $i; done) $(for i in ${TOPDIR}/e_dbus/src/lib/{bluez,connman0_7x,dbus,hal,notification,ofono,ukit}; do echo -I $i; done) -I ${TOPDIR}/eet/src/lib/ -I ${TOPDIR}/evas/src/lib/ -I ${TOPDIR}/edje/src/lib -I ${TOPDIR}/efreet/src/lib -I src/bin/ -I src/modules/illume2/
	# missing headers:
	...
	#  Exchange.h, evry_api.h
		...
		;;
	TMP/st/elementary)
		call_spatch $d -I ${TOPDIR}/eina/src/include/ $(for i in ${TOPDIR}/ecore/src/lib/ecore*; do echo -I $i; done) $(for i in ${TOPDIR}/e_dbus/src/lib/{bluez,connman0_7x,dbus,hal,notification,ofono,ukit}; do echo -I $i; done) -I ${TOPDIR}/eet/src/lib/ -I ${TOPDIR}/evas/src/lib/ -I ${TOPDIR}/edje/src/lib -I ${TOPDIR}/efreet/src/lib -I ${TOPDIR}/PROTO/libeweather/src/lib/ -I ./ -I src/lib/ 
		;;
	E16)
	...
	E-MODULES-EXTRA)
		...
		call_spatch $d $(for i in ${TOPDIR}/ecore/src/lib/ecore*; do echo -I $i; done) $(for i in ${TOPDIR}/e_dbus/src/lib/{bluez,connman0_7x,dbus,hal,notification,ofono,ukit}; do echo -I $i; done) -I ${TOPDIR}/e/src/bin/
		;;
	*)
		# !@##$&#@$##&% -- don't bother about includes, let's hope it works
		call_spatch $d $(for i in ${TOPDIR}/ecore/src/lib/ecore*; do echo -I $i; done) $(for i in ${TOPDIR}/e_dbus/src/lib/{bluez,connman0_7x,dbus,hal,notification,ofono,ukit}; do echo -I $i; done) -I ${TOPDIR}/eina/src/include/ -I ${TOPDIR}/eet/src/lib/ -I ${TOPDIR}/evas/src/lib/ -I ${TOPDIR}/edje/src/lib
		;;
	esac
done
